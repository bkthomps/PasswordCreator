...
then
	echo "allow_lowercase must be 0 or 1"
	exit 1
fi

if [[ $allow_uppercase -ne 0 && $allow_uppercase -ne 1 ]]
then
        echo "allow_uppercase must be 0 or 1"
        exit 1
fi

if [[ $allow_numbers -ne 0 && $allow_numbers -ne 1 ]]
then
        echo "allow_numbers must be 0 or 1"
        exit 1
fi

if [[ $allow_special_characters -ne 0 && $allow_special_characters -ne 1 ]]
then   
        echo "allow_special_characters must be 0 or 1"
        exit 1
fi
...
then
	echo "no characters are allowed... nothing can be generated"
	exit 1
fi

if [[ $size -lt 1 ]]
then
	echo "size is less than 1... nothing can be generated"
	exit 1
fi
